...
python repressed_response_Pijs_meanvals.py ${data_fname} ${num_perturbation_samples} ${num_control_samples} ${unweighted_nw_fname} ${percentile} ${path_length_thresh} ${num_trials} ${out_dir}/${response_nw_fname} ${out_dir}/temp/Pij
...
python try_different_thresholds_repressed_response_meanvals.py ${data_fname} ${perturbation_sample} ${control_sample} ${unweighted_nw_fname} ${path_length_thresh} ${out_dir}/Pij_zscores.txt ${out_dir}/thresh_TopNet_sizes.txt
