...
# node_weight is a pandas dataframe, indexed by gene, having only 1 column, the node weight itself
def get_weighted_network(node_weight, nw):
	weighted_nw = nx.DiGraph()
	for edge in nw.edges():
		if edge[0] in node_weight.index and edge[1] in node_weight.index: # got node weight values
			if node_weight.loc[edge[0]][0] > 0 and node_weight.loc[edge[1]][0] > 0: # node weight is positive for both nodes
				n1 = node_weight.loc[edge[0]][0]
				n2 = node_weight.loc[edge[1]][0]
				weighted_nw.add_edge(edge[0], edge[1], weight = get_edge_weight(n1, n2))
	return weighted_nw
