...
def get_randomized_node_weight_matrix(data):
	return get_randomized_colnum(data, 0)
...
############## Methods for the case with multiple conditions, node weight precomputed

def restructure_node_weight(data, condition):
	condition_data = data.loc[:, condition]
	data = data.drop([condition], axis = 1)
	data_restructured = pd.concat([condition_data, data], axis = 1)
	return data_restructured
		
# We are only interested in column 0 (condition of interest)
# So we return a pandas dataframe with only the relevant column
def get_relevant_node_weight(data):
...
# Here we have k1 perturbed samples and k2 control samples
# k1 + k2 = m (total number of samples)
# We return the mean of the first k1 columns as the representative perturbed sample
# and the mean of the next k2 columns as the representative control sample
def get_mean_SI(SI, num_perturbed_samples, num_control_samples):
        perturbed_mean = SI.iloc[:,0:num_perturbed_samples].mean(1)
        control_mean = SI.iloc[:,num_perturbed_samples:(num_control_samples+num_perturbed_samples)].mean(1)
        SI_mean = pd.concat([perturbed_mean, control_mean], axis=1)
        return SI_mean